...
function formatAgetList(data, status) {
    ...
    var html = "";
    html +=
        '<div class="row box locate-driver-onmap" data-lat="' +
        data.location_lat +
        '" data-lng="' +
        data.location_lng +
        '" >';
    ...
    html +=
        '<div class="top10"><i class="ion-ios-circle-filled ' +
        class_name +
        ' "></i></div> ';
    html += "</div> <!--col-->";
    ...
    if (!empty(data.device_platform)) {
        html +=
            '<p class="connection_status" style="font-weight: bold;">' +
...
function fillAgentDashboard(data) {
    ...
    if (data.details.total.length > 0) {
        ...
        $.each(data.details.total, function (key, val) {
            html_offline += formatAgetList(
                val,
                val.is_online == 1 ? jslang.online : jslang.offline
            );
        });
