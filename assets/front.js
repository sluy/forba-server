    ...
    $.validate({
        ...
        onSuccess: function () {
            window.location.href =
                home_url + "/pricing/?email=" + $("#email_address").val();
            return false;
        },
    });
        ...
        success: function (data) {
            ...
            if (data.code == 1) {
                switch (action) {
                    ...
                    case "customerRating":
                        //alert(data.msg);
                        $(".track-rating-wrap").html(
                            '<h3 style="margin-top:40%;">' + data.msg + "</h3>"
                        );
                        break;
...
$(document).ready(function () {
    ...
    $(".modal-open-onload").each(function (m) {
        m.modal("show");
    });
    // Downloads
    ...
    track_ajax = $.ajax({
        ...
        success: function (data) {
            ...
            if (data.code == 1) {
                ...
                if (
                    $("#task_status").val() == "cancelled" ||
                    $("#task_status").val() == "failed" ||
                    $("#task_status").val() == "declined"
                ) {
                    ...
                    $("#task_status").val() == "successful" &&
                    task_info.driver_id > 0
                ) {
                    $(".track-arrived-wrap").hide();
                    $(".track-contact-wrap").hide();
                    $(".track-message").hide();
                ...
                if (task_info.driver_id > 0) {
                    if (map_provider == "google") {
                        if (!empty(track_marker_driver)) {
                            ...
                            track_marker_driver.setPosition(
                                new google.maps.LatLng(
                                    task_info.driver_location_lat,
                                    task_info.driver_location_lng
                                )
                            );
                            $(".no-agent-p").hide();
                            $(".trackdetails-wrap").html(
                                task_info.track_details
                            );
                            ...
                            if (
                                task_lat != task_info.task_lat &&
                                task_lng != task_info.task_lng
                            ) {
                                window.location.reload();
                            }
                            ...
                            if (
                                dropoff_task_lat !=
                                    task_info.dropoff_task_lat &&
                                dropoff_task_lng != task_info.dropoff_task_lng
                            ) {
                                window.location.reload();
                            }
                        } else {
