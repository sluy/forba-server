    ...
    <div class="col-md-8 border col">
        <div class="inner border">
	     ...
	     <img src="<?php echo Yii::app()->getBaseUrl(true) . "/assets/images-front/phone-icon.png"; ?>">-->
                ...
                <a href="/downloads/driver.apk"><i class="ion-social-android"></i></a>
            </div>
...
<div class="meeting-section">
    ...
    <div class="content">
        <div class="subtitle">
            Te apoyamos en todo momento
        </div>
        <div class="title">
            Reúnete con nosotros
        </div>
        <div class="message">
            De la mano de los mejores profesionales guiaremos tu negocio a la excelencia.
            <br>
            ¿Tienes dudas? ¿No sabes cómo encaminar tu negocio?
            <br />
            Agenda una reunión online con nuestro capacitado equipo para atenderte a la medida que mereces.
        </div>

        <a class="meet-link" href="https://calendly.com/forba" target="_blank">
            AGENDAR LA REUNIÓN AHORA
            <i class="ion-ios-arrow-thin-right"></i>
        </a>

        <div class="message">
            <b>Es totalmente grátis</b>, ¿Qué esperas?
        </div>
    </div>
...
<div class="sections section-4">
    ...
    <form class="row custom-pricing-box" method="POST" action="contact.php">
        <div class="col-xs-12 text-center" style="padding-bottom:30px;">
            <div class="subtitle">
                ¿Ningún plan cubre su negocio?
            </div>
            <div class="title">
                Contacte con nosotros
            </div>
            <p>
                ¿Eres una marca tiene más de 500 ejecutivos o
                flotas? ¿Su negocio es personalizado y necesitas atención
                a medida la marca que representas?
                <br>
                No te preocupes en <b>forba</b> te atenderemos adecuadamente.
                Envía un mensaje por medio del siguiente formulario para atender
                tu caso, dudas o sugerencias.
            </p>
        </div>
        <div class="col-xs-12 col-md-6">
            <div class="form-group">
                <label for="contact-company">Nombre de la Empresa</label>
                <input type="text" class="form-control contact-field" id="contact-company" name="company" placeholder="MiCompañía CA" max-length="255" onchange="ValidateContactForm()" onkeyup="ValidateContactForm()">
            </div>
            <div class="form-group">
                <label for="contact-name">Nombre de Contacto</label>
                <input type="text" class="form-control contact-field" id="contact-name" name="name" placeholder="John Doe" max-length="255" onchange="ValidateContactForm()" onkeyup="ValidateContactForm()">
            </div>
            <div class="form-group">
                <label for="contact-email">Correo electrónico de Contacto</label>
                <input type="email" class="form-control contact-field" id="contact-email" name="email" placeholder="johndoe@email.com" max-length="255" onchange="ValidateContactForm()" onkeyup="ValidateContactForm()">
            </div>
            <div class="form-group">
                <label for="contact-phone contact-field">Teléfono de Contacto</label>
                <input type="text" class="form-control" id="contact-phone" name="phone" placeholder="2125555555" max-length="255" onchange="ValidateContactForm()" onkeyup="ValidateContactForm()">
            </div>
        </div>
        <div class="col-xs-12 col-md-6">
            <div class="form-group">
                <label for="contact-message">Tópico</label>
                <select class="form-control contact-field" name="topic" id="contact-topic" max-length="255" onchange="ValidateContactForm()" onkeyup="ValidateContactForm()">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control contact-field" rows="8" id="contact-message" name="message" max-length="1000" onchange="ValidateContactForm()" onkeyup="ValidateContactForm()" placeholder="Escriba aquí de manera específica su solicitud/sugerencia."></textarea>
            </div>
        </div>
        <div class="col-xs-12 text-center" style="padding: 20px;">
            <button type="submit" id="contact-submit" class="btn btn-success" disabled="disabled">Enviar Mensaje</button>
        </div>
    </form>
    ...
    <script type="text/javascript">
        function ValidateContactForm() {
            ...
            document.querySelectorAll('.contact-field').forEach(
                function(input) {
                    if (!input.value || typeof input.value !== 'string' || input.value.trim().length < 1) {
                        ok = false;
                    }
                }
            )
        ...
        $contactModal = [
            ...
            'message' => 'No se pudo enviar el mensaje; intente nuevamente en algunos minutos.',
        ];
        if (isset($_GET) && array_key_exists('contact_sended_status', $_GET)) {
            $showModal = true;
            if ($_GET['contact_sended_status'] === 'success') {
                $contactModal['title'] = 'Éxito';
                $contactModal['class'] += ' modal-open-onload';
                $contactModal['message'] = 'Mensaje enviado satisfactoriamente.';
            }
        }
        if ($showModal) {
            echo "\t\t alert('" . $contactModal['message'] . "');";
        }
        ?>
    </script>
</div>
